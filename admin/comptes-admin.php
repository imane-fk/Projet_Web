...
$errorTitle = '';
$users = [];
function FetchAdmins($pdo, $searchKey, &$errorMessage, &$errorTitle)
{
    try {
        $query = "
    ...
    ";
        if (!empty($searchKey)) {
            $query .= " AND (LOWER(u.Nom) LIKE :searchKey 
                     ...
                     OR LOWER(a.Email_Universitaire) LIKE :searchKey)
                     OR LOWER(a.Role) LIKE :searchKey
                     ";
        }
...
if (!isset($pdo)) {
    ...
    $users = FetchAdmins($pdo, '', $errorMessage, $errorTitle);
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['search'])) {
            $searchKey = filter_var($_POST['search'], FILTER_SANITIZE_STRING);
            $users = FetchAdmins($pdo, $searchKey, $errorMessage, $errorTitle);
        } else if (isset($_POST['Edit'])) {
            $userId = $_POST['Edit'];
            header("Location: page-admins.php?userId=$userId");
        } else
        ...
        if (isset($_POST['Nom']) && isset($_POST['Prenom']) && isset($_POST['email']) && isset($_POST['Role']) && isset($_POST['Date_Prise_Fonction']) && isset($_POST['Date_Fin_Fonction']) && isset($_POST['Heures_Travail']) && isset($_POST['Statut'])) {
            $Nom = $_POST['Nom'];
            $Prenom = $_POST['Prenom'];
            $email = $_POST['email'];
            ...
            $Role = $_POST['Role'];
            $Date_Prise_Fonction = $_POST['Date_Prise_Fonction'];
            $Date_Fin_Fonction = $_POST['Date_Fin_Fonction'];
            $Heures_Travail = $_POST['Heures_Travail'];
            try {
                $stmt = $pdo->prepare("INSERT INTO Utilisateur (Nom, Prenom, Email, Password, Date_Creation, Statut, Profil) VALUES (:Nom, :Prenom, :email, :password, :Date_Creation, :Statut, 'adm')");
                ...
                if ($stmt->execute()) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO administration (ID_User, Role, Date_Prise_Fonction, Date_Fin_Fonction, Heures_Travail) VALUES (:ID_User, :Role, :Date_Prise_Fonction, :Date_Fin_Fonction, :Heures_Travail)");
                    $stmt->bindParam(':ID_User', $ID_User);
                    $stmt->bindParam(':Role', $Role);
                    $stmt->bindParam(':Date_Prise_Fonction', $Date_Prise_Fonction);
                    $stmt->bindParam(':Date_Fin_Fonction', $Date_Fin_Fonction);
                    $stmt->bindParam(':Heures_Travail', $Heures_Travail);
                    if ($stmt->execute()) {
                        $mail = getMailer();
    ...
    <div class="p-4 sm:ml-64">
        ...
        <div id="drawer-form" class="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-form-label">
            ...
            <form method="post" class="mb-6" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                ...
                <div class="mb-6">
                    <label for="Role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Date Prise Fonction</label>
                    <input type="date" name="Date_Prise_Fonction" id="" required>
                </div>
                <div class="mb-6">
                    <label for="Role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Date Fin Fonction</label>
                    <input type="date" name="Date_Fin_Fonction" id="" required>
                </div>

                <div class="mb-6">
                    <label for="Role" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Heures Travail</label>
                    <input type="number" name="Heures_Travail" id="Heures_Travail" required>
                </div>
                ...
                <tbody>
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <div class="flex items-center">
                            <div class="h-2.5 w-2.5 rounded-full me-2"></div> ' . $user['Role'] . '
                        </div>
                    </td>';
